...
public class BlockBankAccountVerificatorTest {
    @Autowired
    ...
    public void createCardVerificatorTest() {
        ...
        AbstractVerificator verificator = verificatorAbstractFactory.getBlockBankAccountVerificator(VerificationType.EMAIL);
        assertEquals(verificator.getClass(), BlockBankAccountVerificator.class);

        BankAccount bankAccount = new BankAccount();
        RequestOrder blockBankAccountRequest = requestFactory.createBlockBankAccountRequest(bankAccountService, bankAccount);
        String id = verificator.startVerification(blockBankAccountRequest);

        assertFalse(logger.isEmpty());
        assertTrue(logger.get(0).contains("block") && logger.get(0).contains("code"));
        logger.clear();
    }
}
